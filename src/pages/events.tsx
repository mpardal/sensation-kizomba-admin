...
function EventsPage() {
  ...
  return (
    <Container maxW="8xl">
      <Flex as="section" direction="column" alignItems="flex-start" bg="gray.700" borderRadius="2xl" px={6} py={2}>
        <EventPageBreadcrumb />
            ...
            <Table variant="simple">
              ...
              <Tbody>
                {eventQuery.data.docs.map((event) => {
                  ...
                  return <Tr key={event.id} aria-label={data.title}>
                    <Td aria-label="actions sur les événements" textAlign="center">
                      <IconButton size="xs" aria-label="éditer l'événement" role="navigation" icon={<EditIcon />} colorScheme="gray" bg="gray.600 " />
                    </Td>
                    <Td aria-label="nom de l'événement">{data.title}</Td>
                    <Td aria-label="date de début de l'événement">{data.date.from.toMillis()}</Td>
                    <Td aria-label="date de fin de l'événement">{(data.date.to ?? data.date.from).toMillis()}</Td>
